...
const openExternal = url => { if(!url) return; window.location.href = url; };
...
const actions = {
  ...
  regras: [
    ...
    hotspot('Ver regras completas (PDF)',pct(49.0,82.4,45.0,5.6),()=>showImage('assets/extras/regras-completas.png','Regras completas')),
  ...
  rota: [
    ...
    hotspot('Abrir Rota do Vinho no mapa',pct(4.5,86.1,57.5,5.2),()=>openExternal(C.external.rotaMaps)),
    hotspot('Mapa do Roteiro do Vinho',pct(63.0,86.1,32.5,5.2),()=>showImage('assets/extras/mapa-roteiro.png','Mapa do Roteiro do Vinho')),
    hotspot('Voltar para o menu',pct(20.4,92.2,59.2,5.0),()=>go('home'))
  ],
  delivery: [
    hotspot('Voltar',pct(2,1,10,6),()=>go('home')),
    hotspot('Born In Smoke',pct(61.5,29.0,31.5,3.9),()=>openExternal(C.external.bornWhatsapp)),
    hotspot('Itacolomy',pct(61.5,48.0,31.5,3.9),()=>openExternal(C.external.itacolomyWhatsapp)),
    hotspot('Di Pomodoro',pct(61.5,66.2,31.5,3.9),()=>openExternal(C.external.diPomodoroWhatsapp)),
  ...
  checkout: [
    hotspot('Voltar',pct(2,2,10,7),()=>go('home')),
    hotspot('Ver foto do portão',pct(73.5,68.3,18.5,4.4),()=>showPendingPhoto(),false,'Ver foto'),
...
function boxToPixels(box, w, h){
  ...
  return {
    x: w * num(box.left) / 100,
    y: h * num(box.top) / 100,
    width: w * num(box.width) / 100,
    height: h * num(box.height) / 100
  };
}
function activateHotspot(action, ev){
  ev.preventDefault();
  ev.stopPropagation();
  action();
}
function render(){
  ...
  svg.classList.add('hotspot-layer');
  svg.setAttribute('viewBox',`0 0 ${s.ratio[0]} ${s.ratio[1]}`);
  svg.setAttribute('preserveAspectRatio','none');
  svg.setAttribute('aria-hidden','false');
  const debug=new URLSearchParams(location.search).get('debug')==='1';

  (actions[id]||[]).forEach(h=>{
    if(h.hidden || h.visibleLabel) return;
    const b=boxToPixels(h.box,s.ratio[0],s.ratio[1]);
    ...
    r.setAttribute('aria-label',h.label);
    r.classList.add('hotspot-svg');
    if(debug) r.classList.add('debug');
    let fired=false;
    r.addEventListener('pointerup',ev=>{ fired=true; activateHotspot(h.action,ev); setTimeout(()=>fired=false,350); });
    r.addEventListener('click',ev=>{ if(fired){ ev.preventDefault(); return; } activateHotspot(h.action,ev); });
    r.addEventListener('keydown',ev=>{ if(ev.key==='Enter'||ev.key===' '){ activateHotspot(h.action,ev); } });
    svg.appendChild(r);
  });
  stage.appendChild(svg);

  // Visible action used only on checkout item 7: draw as HTML so its label remains readable.
  (actions[id]||[]).filter(h=>h.visibleLabel&&!h.hidden).forEach(h=>{
    const b=document.createElement('button');
    b.type='button';
    b.className='hotspot visible-action';
