...
function showImage(src,alt){ openModal(`<img src="${src}" alt="${alt}">`); }
function showBreakfastPhoto(){ openModal(`<div class="gallery"><img src="assets/extras/casinha-vermelha.png" alt="Casinha vermelha para retirada do café da manhã"><div class="caption">Retire o café da manhã na casinha vermelha, no pé da escada.</div></div>`); }
function showCasaAzulDirections(){ openModal(`<div class="gallery"><img src="assets/extras/casa-azul-portao.png" alt="Vista da Casa Azul a partir do portão"><img src="assets/extras/casa-azul-fachada.png" alt="Fachada da Casa Azul"><div class="caption"><strong>Como chegar:</strong> olhando do portão da cabana, esta é a Casa Azul. Para chegar, suba a rua à esquerda. A senha da porta é <strong>2381</strong>. Por favor, feche a porta ao sair.</div></div>`); }
function showDreamBenefit(){ openModal(`<div class="gallery"><img src="assets/extras/dream-car-beneficio.png" alt="Benefício Dream Car Museum"><div class="caption"><strong>Cupom:</strong> CABANAROTADOVINHO<br><br><button type="button" onclick="window.open('${C.external.dreamCarPurchase}','_blank','noopener,noreferrer')">Comprar no site da Dream Car</button></div></div>`); }
...
function showCheckoutEvaluation(){
  openModal(`<div class="gallery checkout-evaluation"><img src="assets/extras/avaliacao-pos-checkout.png" alt="Informações sobre avaliação da hospedagem"><button type="button" class="modal-home-btn" id="checkoutHomeBtn">Voltar para a tela inicial</button></div>`);
